...
/*		for(int i=0; i<targetUrlList.length; i++){
		...
		*/
		...
		System.out.println("Finish getting all courses url, Let's crawl review data");
...
//			tool.crawlData(urlString, url_id);
		...
		if (document != null) {
			...
			StreamResult xmlFile = new StreamResult(new File("ClassCentral_reviewData"+format.format(now)+".xml"));
