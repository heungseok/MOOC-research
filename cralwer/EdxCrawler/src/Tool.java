...
public class Tool {
	...
	HashMap<String, String> schoolMap;
...
//		driver.manage().window().maximize();
	...
	public void parseSchoolData() throws InterruptedException{
		
		driver.navigate().to("https://www.edx.org/schools-partners");
		Thread.sleep(2000);
		
		schoolMap = new HashMap<String, String>();
		
		ArrayList<WebElement> members = new ArrayList<WebElement>(
				driver.findElements(By.cssSelector("div.member-logo")) );
		
		ArrayList<String> school_url = new ArrayList<String>();
		
		for(WebElement element: members){
			String url_base = element.findElement(By.tagName("a")).getAttribute("href");
			school_url.add(url_base);

		}
		Thread.sleep(500);
		
		for(String url: school_url){
			driver.navigate().to(url);
			Thread.sleep(1000);
			
			String temp [] = url.split("/");
			String key = temp[temp.length-1];
			temp = driver.findElement(By.cssSelector("span.subtitle")).getText().split("from");
			String value = temp[temp.length-1];
			System.out.println(key + "%" + value);
			// add both url and school's token as a key of school's map data structure.
			schoolMap.put(key, value);
			schoolMap.put(url, value);
	...
	public ArrayList<String> getVideoURL() throws IOException, Exception{
		
		driver.navigate().to("http://edx.org/courses/?course=all");
		Thread.sleep(1000);
...
//			count++;	
		...
		for(int i=0; i<size_of_url; i++){
			...
			// xseries 媛뺤쓽�뱾�� �젣�쇅�븯怨� list�뿉 �떞�뒗�떎.
			if(url.contains("xseries") || url.contains("micromasters") || url.contains("professional-certificate")){
				continue;
			}else{
				urlList.add(url);
